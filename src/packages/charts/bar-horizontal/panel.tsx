...
import {
  cloneHorizontalBarSpec,
  createDefaultHorizontalBarSpec,
  getHorizontalBarDataRows,
} from "./config";

type HorizontalBarChartPanelProps = {
  widget: Widget;
...
function getFieldOptions(spec: IBarChartSpec) {
  const rows = getHorizontalBarDataRows(spec);
...
export function HorizontalBarChartPanel({
  ...
  const barWidth = typeof spec.barWidth === "number" ? spec.barWidth : 20;
  ...
  const updateSpec = (mutator: (nextSpec: IBarChartSpec) => void) => {
    const nextSpec = cloneHorizontalBarSpec(spec);
    mutator(nextSpec);
  ...
  return (
    <div className="space-y-3">
      <CollapseSection
        title="坐标轴显示"
        description="横向柱状图仍然沿用 bar 的双轴配置，只是方向改为 horizontal。"
        badge="Axis"
        contentClassName="space-y-3"
      >
        <ToggleSwitch
          label="显示 X 轴"
          description="控制底部数值轴和网格显示。"
      ...
      <CollapseSection
        title="柱形样式"
        description="先开放柱宽和标签，用于验证横向布局链路稳定。"
        badge="Style"
        contentClassName="space-y-3"
      >
        <NumberStepper
          label="柱宽"
          description="直写 spec.barWidth。"
          value={barWidth}
          min={8}
          max={64}
          step={1}
      ...
      <CollapseSection
        title="字段映射"
        description={`当前数据共 ${rowCount} 行，直写 spec.xField / spec.yField / spec.seriesField。`}
        badge="Data"
        contentClassName="space-y-3"
      >
        <SelectField
